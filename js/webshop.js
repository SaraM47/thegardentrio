  ...
  if (document.getElementById("basket")) {
    ...
    if (basketItems.length > 0) {
      ...
      for (let i = 0; i < basketItems.length; i++) {
        ...
        let newItemNameText = document.createTextNode(
          basketItems[i].artName + ", "
        );
        ...
        let newItemCountText = document.createTextNode(
          basketItems[i].nums + " st "
        );
...
function showCheckout() {
  if (document.getElementById("checkout")) {
    ...
    if (basketItems.length > 0) {
      ...
      checkoutEl.innerHTML +=
        "<tr>" +
        "<td colspan='5' class='checkout-sum'>Amount: " +
        sum +
        ":-</td>";
    } else {
      // Tomt i listan
      checkoutEl.innerHTML =
        "<tr><td colspan='5'>The shopping cart is empty</td></tr>";
    }
  }
}
...
function checkoutBasket() {
  ...
  if (basketItems != null && basketItems.length > 0) {
    ...
    if (itemCount == 1) {
      orderMessage =
        "Your order has been received! One item - total amount: " +
        totalSum +
        ":-";
    } else {
      orderMessage =
...
function updateCartBadge() {
  ...
  // Hämtar element  där antalen i varukorgen ska visas och uppdaterar både nav-cart-count och header-cart-count
  ...
  const headerCartLinkEl = document.querySelector(".header-cart-link");

  // Uppdaterar antalet i nav-cart-count och header-cart-count, och visar eller döljer dem baserat på om det finns varor i varukorgen eller inte
  if (navCartCountEl) {
    navCartCountEl.textContent = numOfItems;
    navCartCountEl.style.visibility = numOfItems > 0 ? "visible" : "hidden";
  }

  if (headerCartCountEl && headerCartLinkEl) {
    headerCartCountEl.textContent = numOfItems;
    headerCartLinkEl.classList.toggle("visible", numOfItems > 0);
  }
}
